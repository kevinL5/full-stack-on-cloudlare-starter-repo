	...
	async run(event: Readonly<WorkflowEvent<DestinationStatusEvaluationParams>>, step: WorkflowStep) {
		...
		const evaluationInfo = await step.do(
			...
			async () => {
				...
				await this.env.EVALUATIONS_BUCKET.put(r2PathHtml, data.html);
				await this.env.EVALUATIONS_BUCKET.put(r2PathBodyText, data.bodyText);
				await this.env.EVALUATIONS_BUCKET.put(r2PathScreenshot, screenshotBuffer);
